...
try {
    ...
    // Consulta SQL para obtener todas las publicaciones con 'vista' igual a 'visible'
    if ($metodo == 'GET') {
        $sqlSelect = "SELECT * FROM publicaciones WHERE vista = :vista";
        $sentencia = $conexion->prepare($sqlSelect);

        // Asignar el valor al parámetro de la consulta
        $vista = 'visible';
        $sentencia->bindParam(':vista', $vista);
        ...
        if ($sentencia->execute()) {
            ...
            echo json_encode(["publicaciones" => $resultado]);
        } else {
